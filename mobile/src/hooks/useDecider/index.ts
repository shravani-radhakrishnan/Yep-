...
import { CATEGORIES, makeCategory } from '../../lib/categories';
import { subscribeToUserData, saveUserLists, saveUserCategories } from '../../lib/firebase/firestoreService';
import { loadData, saveData, makeItem, loadCustomCategories, saveCustomCategories } from '../../lib/storage';
...
export function useDecider(userId: string | null = null) {
  ...
  useEffect(() => {
    if (isFirestore) {
      ...
      Promise.all([loadData(), loadCustomCategories()]).then(([d, cats]) => {
        setData(d);
        setCustomCats(cats);
        setReady(true);
      });
  ...
  const persist = useCallback((updater: (cur: DataStore) => DataStore) => {
    setData(cur => {
      ...
      if (userId) saveUserLists(userId, next);
      else saveData(next);
      return next;
    });
  }, [userId]);
  ...
  const pickOne = useCallback((onPick?: () => void) => {
  ...
  const giveFeedback = useCallback((status: Status) => {
    ...
    showToast(status === 'done' ? '✓ Done!' : 'Noted — skipped');
